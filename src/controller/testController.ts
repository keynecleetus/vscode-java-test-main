...
export function createTestController(): void {
    ...
    testController = tests.createTestController('java', 'Revature Java Test');

    testController.resolveHandler = async (item: TestItem) => {
        await loadChildren(item);
    };

    testController.createRunProfile('Run Revature Tests', TestRunProfileKind.Run, runHandler, true, runnableTag);
    testController.createRunProfile('Debug Revature Tests', TestRunProfileKind.Debug, runHandler, true, runnableTag);
